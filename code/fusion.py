...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        '''
        curpoints = self.points[indices]
        curnormals = self.normals[indices]
        curcolors = self.colors[indices]
        curweights = self.weights[indices]
        
        points = (R @ points.T + t).T
        normals = (R @ normals.T).T
        
        avg_points = (curpoints * curweights + points) / (curweights + 1)
        avg_normals = (curnormals * curweights + normals) / (curweights + 1)
        avg_colors = (curcolors * curweights + colors) / (curweights + 1)
        avg_weights = curweights + 1
        avg_normals = avg_normals / np.linalg.norm(avg_normals, axis=1)[:, None]
        self.points[indices] = avg_points
        self.normals[indices] = avg_normals
        self.colors[indices] = avg_colors
        self.weights[indices] = avg_weights
    ...
    def filter_pass1(self, us, vs, ds, h, w):
        ...
        # conditions to ensure valid correspondence
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        '''
        mask = np.zeros_like(points).astype(bool)
        # conditions to ensure valid correspondence
        mask = np.linalg.norm(points - input_points, axis=1) < dist_diff
        norm_normals = np.linalg.norm(normals, axis=1)
        norm_input_normals = np.linalg.norm(input_normals, axis=1)
        normals = normals / norm_normals[:, None]
        input_normals = input_normals / norm_input_normals[:, None]
        dotprod = np.sum(normals * input_normals, axis=1)
        normal_mask = (np.arccos(np.clip(np.abs(dotprod), -1, 1)) < angle_diff)
...
if __name__ == '__main__':
    ...
    intrinsic_struct = o3d.io.read_pinhole_camera_intrinsic('E:\dev\slam_icpp\code\intrinsics.json')
    ...
    vis.create_window()
    vis.add_geometry(global_pcd.transform(o3d_utility.flip_transform))
    vis.run()
    vis.capture_screen_image(fr'E:\dev\slam_icpp\images\fusion/{args.start_idx}_{args.end_idx}.png')
