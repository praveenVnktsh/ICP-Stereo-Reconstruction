...
def find_projective_correspondence(source_points,
    ...
    mask = np.zeros_like(target_us).astype(bool)
    mask = np.linalg.norm(T_source_points - target_vertex_map[target_vs, target_us], axis=1) < dist_diff
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    # for i in range(M):
    #     A[i, :3] = np.cross(p_prime[i], n_q[i])
    #     A[i, 3:] = n_q[i]
    #     b[i] = np.dot(n_q[i], q[i] - p_prime[i])
    
    # vectorized:
    A[:, :3] = np.cross(p_prime, n_q)
    A[:, 3:] = n_q
    b = np.einsum('ij,ij->i', n_q, q - p_prime)
    # End of TODO
...
def icp(source_points,
    ...
    for i in range(10):
        ...
        # Debug, if necessary
        if debug_association and i % 10 == 0:
...
if __name__ == '__main__':
    import os
    os.makedirs('E:\dev\slam_icpp\images/icp', exist_ok=True)
    ...
    intrinsicpath = 'E:\dev\slam_icpp\code\intrinsics.json'
    intrinsic_struct = o3d.io.read_pinhole_camera_intrinsic(intrinsicpath)
    ...
    o3d_utility.visualize_icp(source_points, target_vertex_map.reshape(
        (-1, 3)), np.eye(4), 
        f"E:\dev\slam_icpp\images/icp/pre_icp_{args.source_idx}_{args.target_idx}.png")
    ...
    o3d_utility.visualize_icp(source_points, target_vertex_map.reshape(
        (-1, 3)), T,
        f"E:\dev\slam_icpp\images/icp/post_icp_{args.source_idx}_{args.target_idx}.png")
